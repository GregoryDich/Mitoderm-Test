...
export default function CourseSelection({ 
  ...
  // Связываем выбранную карточку с variant
  ...
  return (
    <section 
      ref={sectionRef}
      className="relative overflow-hidden bg-gradient-to-b from-[#0f1829] to-[#1a2744] py-20" 
      dir="rtl"
    >
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4">
        ...
        >
          <h2 className="bg-gradient-to-r from-[#fcd384] via-[#dfba74] to-[#be800c] bg-clip-text text-4xl font-bold text-transparent md:text-5xl">
            {heading}
          </h2>
        </motion.div>
          ...
          {courses.map((course, index) => {
            ...
            return (
              <motion.div
                ...
                className={`group relative w-full max-w-sm cursor-pointer overflow-hidden rounded-3xl border-2 transition-all duration-300 ${
                  isSelected 
                    ? 'border-[#dfba74] shadow-2xl shadow-[#dfba74]/30' 
                    : 'border-white/10 hover:border-[#dfba74]/50'
                } bg-gradient-to-br from-[#1a2744]/95 to-[#0f1829]/95 backdrop-blur-sm`}
              >
                {/* Background gradient */}
                ...
                {isFeatured && (
                  <div className="absolute left-4 top-4 z-10">
                    <motion.div
                      ...
                      className="rounded-full bg-gradient-to-r from-[#dfba74] to-[#be800c] px-3 py-1 text-xs font-bold text-white shadow-lg"
                    >
                      מומלץ
                    </motion.div>
                  </div>
                ...
                <div className="relative z-10 p-4 pb-0">
                  <div className={`inline-flex rounded-full bg-gradient-to-r ${course.color} px-4 py-2 text-xs font-bold text-white shadow-lg`}>
                    {course.badge}
                  </div>
                </div>

                {/* Professional Image - Full width, cropped from top */}
                ...
                <div className="relative z-10 p-6">
                  ...
                  <div className="mb-6 space-y-3">
                    {course.description.map((item, i) => (
                      <div key={i} className="flex items-start gap-3">
                        <span className="mt-1 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-[#dfba74]" />
                  ...
                  <motion.button
                    ...
                    className={`w-full rounded-full bg-gradient-to-r ${course.color} px-6 py-3 font-bold text-white shadow-lg transition hover:shadow-xl`}
                  >
                    הרשמה
                  </motion.button>
                </div>
